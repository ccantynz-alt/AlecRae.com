...
export class SmtpConnectionHandler {
  ...
  getGreeting(): SmtpResponse {
    this.state = "ready";
    return { code: 220, message: `${this.config.hostname} ESMTP Emailed Inbound - ${this.session.id}` };
  }

  async processCommand(line: string): Promise<SmtpResponse> {
    if (this.state === "closed") return { code: 421, message: "Connection closed", close: true };
    const trimmed = line.trim();
    if (trimmed.length === 0) return { code: 500, message: "Syntax error, command unrecognized" };
    const spaceIdx = trimmed.indexOf(" ");
    const verb = (spaceIdx > 0 ? trimmed.slice(0, spaceIdx) : trimmed).toUpperCase() as SmtpCommand;
    const args = spaceIdx > 0 ? trimmed.slice(spaceIdx + 1).trim() : "";
    switch (verb) {
      case "EHLO": return this.handleEhlo(args);
      case "HELO": return this.handleHelo(args);
      case "MAIL": return this.handleMailFrom(args);
      case "RCPT": return this.handleRcptTo(args);
      case "DATA": return this.handleDataStart();
      case "RSET": return this.handleReset();
      case "QUIT": return this.handleQuit();
      case "STARTTLS": return this.handleStartTls();
      case "NOOP": return { code: 250, message: "OK" };
      default: return { code: 502, message: `Command not implemented: ${verb}` };
    }
  }
  ...
  async processDataChunk(chunk: Uint8Array): Promise<SmtpResponse | null> {
    ...
    const messageData = combined.slice(0, terminatorIdx + 2);
    const unstuffed = this.unstuffDots(messageData);
    try {
      const envelope: SmtpEnvelope = { mailFrom: this.session.mailFrom ?? "", rcptTo: [...this.session.rcptTo] };
  ...
  private handleEhlo(hostname: string): SmtpResponse {
    if (!hostname) return { code: 501, message: "EHLO requires a hostname" };
    this.session.heloHostname = hostname;
    this.session.clientHostname = hostname;
    this.state = "ready";
    const extensions = [`${this.config.hostname} greets ${hostname}`, `SIZE ${this.config.maxMessageSize}`, "8BITMIME", "SMTPUTF8", "PIPELINING", "ENHANCEDSTATUSCODES"];
    if (!this.session.secure) extensions.push("STARTTLS");
    return { code: 250, message: extensions.join("\n") };
  }

  private handleHelo(hostname: string): SmtpResponse {
    if (!hostname) return { code: 501, message: "HELO requires a hostname" };
    this.session.heloHostname = hostname;
    this.session.clientHostname = hostname;
    this.state = "ready";
  ...
  private handleMailFrom(args: string): SmtpResponse {
    if (this.state !== "ready") return { code: 503, message: "Bad sequence of commands" };
    const match = /^FROM:\s*<([^>]*)>/i.exec(args);
    if (!match || match[1] === undefined) return { code: 501, message: "Syntax error in MAIL FROM" };
  ...
  private handleRcptTo(args: string): SmtpResponse {
    if (this.state !== "mail" && this.state !== "rcpt") return { code: 503, message: "Bad sequence of commands" };
    const match = /^TO:\s*<([^>]+)>/i.exec(args);
    if (!match || match[1] === undefined) return { code: 501, message: "Syntax error in RCPT TO" };
    if (this.session.rcptTo.length >= this.config.maxRecipients) return { code: 452, message: "Too many recipients" };
    const recipient = match[1];
    if (!recipient.includes("@")) return { code: 550, message: "Invalid recipient address" };
  ...
  private handleDataStart(): SmtpResponse {
    if (this.state !== "rcpt") return { code: 503, message: "Bad sequence of commands - need RCPT first" };
    if (this.session.rcptTo.length === 0) return { code: 503, message: "No valid recipients" };
    if (this.config.requireTls && !this.session.secure) return { code: 530, message: "Must issue STARTTLS first" };
    this.state = "data";
    this.dataBuffer = [];
    this.dataSize = 0;
  ...
  private handleStartTls(): SmtpResponse {
    if (this.session.secure) return { code: 503, message: "TLS already active" };
  ...
  private concatenateBuffers(): Uint8Array {
    ...
    let offset = 0;
    for (const buf of this.dataBuffer) { result.set(buf, offset); offset += buf.length; }
    return result;
  }
  ...
  private unstuffDots(data: Uint8Array): Uint8Array {
    ...
    let i = 0;
    while (i < data.length) {
      if (i >= 2 && data[i - 2] === 13 && data[i - 1] === 10 && data[i] === 46 && i + 1 < data.length && data[i + 1] === 46) i++;
      const byte = data[i];
...
export class SmtpReceiver {
  ...
  async start(): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      this.server.listen(this.config.port, () => {
        console.log(`[SmtpReceiver] Listening on ${this.config.hostname}:${this.config.port}`);
        resolve();
      });
    });
  }
  ...
  private handleConnection(socket: net.Socket): void {
    ...
    const handler = new SmtpConnectionHandler(this.config, remoteAddress, remotePort);
    ...
    socket.on("data", async (data) => {
      if (inDataMode) {
        try {
          ...
          if (response) {
            ...
            if (response.close) { socket.end(); return; }
          }
        } catch (err) {
          console.error(`[SmtpReceiver] Error processing data from ${remoteAddress}:`, err);
          inDataMode = false;
      ...
      while ((newlineIdx = lineBuffer.indexOf("\r\n")) !== -1) {
        ...
        try {
          ...
          console.error(`[SmtpReceiver] Error processing command from ${remoteAddress}:`, err);
          socket.write("451 Internal server error\r\n");
        }
      }
    });
    socket.on("timeout", () => { socket.write("421 Connection timed out\r\n"); socket.end(); });
    socket.on("error", (err) => { console.warn(`[SmtpReceiver] Socket error from ${remoteAddress}:`, err.message); });
  ...
  getConnectionCount(): number { return this.activeConnections.size; }
