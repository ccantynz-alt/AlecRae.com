...
function ProgressRing({ progress, size, strokeWidth }: ProgressRingProps): JSX.Element {
  ...
  return (
    <svg width={size} height={size} className="transform -rotate-90" aria-hidden="true">
      ...
      <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="url(#focus-timer-gradient)" strokeWidth={strokeWidth} strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={offset} className="transition-[stroke-dashoffset] duration-1000 ease-linear" />
      <defs>
...
export function FocusModeTimer({ className }: FocusModeTimerProps): JSX.Element {
  ...
  const handlePresetClick = useCallback((minutes: number) => { startTimer(minutes); }, [startTimer]);
  const handleStopClick = useCallback(() => { stopTimer(); }, [stopTimer]);
