...
// ─── Removed 2026-07-29: dead API surfaces that fabricated data ──────────────
//
// AI Inbox Agent, Files browser, Security Center, Email Hygiene, Gamification,
// Push Notifications and Translation used to be declared here as well. Every
// one was DEAD — no page imported them. The live pages import dedicated
// modules instead (api-email-hygiene.ts, api-push-notifications.ts, ...), which
// shadowed these and left the copies here unreachable.
//
// They were not merely unused, they were a landmine. Several called endpoints
// that do not exist and `.catch()`d into hardcoded, plausible-looking values —
// a security score of 85 / "B+" with 3 phishing blocked, an inbox-hygiene
// score of 72 with 47 unread. Anything importing them would have shown a user
// invented numbers as their own data, with no error and no way to tell. That
// is CLAUDE.md issue #73(i)'s finding, which turned out to be one of ten such
// fallbacks in this file rather than the single one recorded.
//
// Deleted rather than repaired: there is no caller to repair for, and leaving
// fabricating helpers lying around is how they get imported later.
