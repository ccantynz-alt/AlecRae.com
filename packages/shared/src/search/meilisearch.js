...
// ─── Configuration ────────────────────────────────────────────────────────
const MEILISEARCH_URL = process.env["MEILISEARCH_URL"] ?? process.env["MEILI_URL"] ?? "http://localhost:7700";
const MEILISEARCH_API_KEY = process.env["MEILISEARCH_API_KEY"] ?? process.env["MEILI_MASTER_KEY"] ?? "";
...
export async function searchEmails(accountId, query, options) {
    ...
    const filterParts = [`accountId = "${accountId}"`];
    ...
    return {
        ...
        totalHits: typeof response.estimatedTotalHits === "number"
