...
| 68 | **Mail-blocking DNS gaps** — mx1/mx2/`_spf`/smtp records missing, bounce CNAME mistargeted. NB: `mail.alecrae.com` staying proxied was never a blocker (it's the webmail host); the MTA identity is the separate `smtp.alecrae.com` | HIGH | 2026-07-13 | MOSTLY FIXED 2026-07-13 — Craig executed the Cloudflare changeset, verified resolving live. **Remaining: PTR for 149.28.119.158 → `smtp.alecrae.com` (Vultr panel, Craig)** — until then FCrDNS fails once sending starts. See mail plan Phase 0. |
...
2. ~~Craig: Phase 0 DNS fixes~~ — **EXECUTED 2026-07-13, verified live**: mx1/mx2/smtp A → 149.28.119.158 (grey), MX 10+20, `_spf` TXT, bounce CNAME → smtp. NB: `mail.alecrae.com` stays PROXIED (it's the webmail app on Jarvis); the MTA identity is `smtp.alecrae.com` (`MTA_HOSTNAME` env). **One item left: Craig changes PTR for 149.28.119.158 → `smtp.alecrae.com` in the Vultr panel.**
