...
| 78 | **No deploy-drift detection** — the box served a July-4 build for 10 days (unpushed on-box hotfix → `--ff-only` refused to pull) and nothing alerted. The drift itself was fixed 2026-07-14 (`git reset --hard origin/main` + redeploy). | HIGH | 2026-07-14 | FIXED 2026-07-20 (code+docs) — `/health` + `/v1/health` now report the running process's commit SHA (`apps/api/src/lib/deploy-info.ts`); `scripts/check-deploy-drift.sh` compares box HEAD to `origin/main` on a 15-min systemd timer and writes a status file the API reads back, flipping `/health` to `degraded` on drift. Setup: `docs/infra/deploy-drift-check.md`. **Still OPEN:** the systemd timer itself isn't installed on Jarvis yet — that's an on-box step, do it in the next deploy session. |
...
6c. ~~Deploy-drift prevention (Known Issue #78)~~ — **code+docs SHIPPED 2026-07-20**; remaining step is installing the systemd timer on Jarvis (`docs/infra/deploy-drift-check.md`) during the next box session.
