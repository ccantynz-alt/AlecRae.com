...
| 115 | **Data integrity / disaster-recovery gaps.** (a) IndexedDB corruption is never detected — `apps/web/lib/offline-store.ts`/`indexeddb-cache.ts` only handle `onerror` on open failure, nothing checks structural integrity or triggers a rebuild; storage-quota monitoring and attachment cache eviction are both entirely absent (`indexeddb-cache.ts`'s `evictOlderThan()` exists but has zero call sites — two parallel, mostly-disconnected local-cache database implementations exist, `alecrae-mail` vs `alecrae_mail`, a separate cleanup worth doing on its own). (b) A DB restore has never actually been drilled — `docs/compliance/soc2/policy-templates/bcp-dr-plan.md` has literal `[INSERT DATE]`/`"Not yet conducted"` placeholders where a real test record should be. (c) The incident-response runbook's own JWT_SECRET-rotation step doesn't warn that the same rotation permanently breaks decryption of every stored OAuth token (issue #80's encryption is keyed off `JWT_SECRET` with no versioning/dual-key transition) — the on-call engineer following that exact runbook for a credential-compromise response gets an unpleasant, undocumented surprise. (d) E2EE key loss has no proactive backup/export flow at all — the only warning is reactive (shown after a key mismatch is already detected), not at the point of risk (enabling encryption for the first time). | HIGH | 2026-07-20 | OPEN |
| 116 | **Billing/compliance gaps beyond Stripe idempotency (fixed this session).** (a) `createCheckoutSession` never checks for an existing active subscription before creating a new Checkout Session — an "Upgrade to Pro" button that doesn't first check current plan risks a second concurrent Stripe subscription (double billing) for an already-subscribed customer. (b) No multi-currency amount handling anywhere in `billing.ts` despite the six-currency business-plan pricing table already in this file — self-acknowledged as "billing wiring pending," not a silent bug, but real exposure the moment business-plan billing ships before this is built. (c) Failed-payment dunning (`recordPaymentFailure`/grace window/downgrade) is fully real, but sends zero user notification at any stage — a customer discovers their card failed only when a Pro feature stops working. (d) No GDPR export or right-to-be-forgotten self-service exists at all — `routes/dpa.ts` is a B2B contract e-signature flow, not a data-subject tool; any Article 15/17 request today is entirely manual/out-of-band. | HIGH ((d) GDPR) / MEDIUM (a,b,c) | 2026-07-20 | OPEN |
...
| 118 | **`lib/ai.ts`'s `aiComplete()` — used by AI triage, `ai-rules.ts`, `ai-intelligence.ts`, and other callers — has the same prompt-injection framing gap fixed in `ai-writing.ts` this session (issue #107), but was deliberately left unfixed.** It's used with more varied message-array shapes across callers not fully surveyed this session; applying the same delimiter-wrapping blindly risked breaking a caller in a way that wouldn't have been caught without a broader audit of every call site first. | MEDIUM | 2026-07-20 | OPEN — needs a survey of all `aiComplete()` callers' message shapes before applying the same fix pattern |
| 119 | **Migration caution: `emails_account_message_id_idx` (migration `0008_overjoyed_risque.sql`, part of issue #107's dedup fix) is a new unique constraint on `(account_id, message_id)`.** It was reasoned safe (idempotent sends dedupe upstream of the DB write; synthetic import IDs are always-fresh UUIDs) but **not verified against live production data** — if any pre-existing duplicate `(account_id, message_id)` pair already exists in the production `emails` table, this migration will fail to apply (safely — `db:migrate` errors out rather than partially applying, per its documented behavior) rather than silently corrupt anything. | LOW | 2026-07-20 | OPEN — run a duplicate-check query against production before the next `db:migrate` on Jarvis includes migration 0008 |
