...
**Current focus:** Second full codebase audit complete (issue #65, 2026-07-04) — ~30 bugs fixed across 16 files. Previously shipped multi-workspace support (issue #63, Craig-authorized) — one login can now own/join several separate workspaces via a `workspace_members` table + `/v1/workspaces` + `/v1/auth/switch-workspace`; **run `bun run db:migrate` on the box for this to take effect in prod.** Also fixed issue #64 (pro/enterprise plan tier not recognized). Full ground-truth code audit run 2026-07-01 (see Known Issues #58-62). Vapron Type 2 migration COMPLETE, `mail.vapron.ai` fully verified (SPF ✅ DKIM ✅ DMARC ✅ MX ✅ Return-Path ✅), smoke test passed. Outstanding Craig tasks: (1) finish 158→161 infra migration (copy `.env`, stand up `alecrae-api`/`alecrae-mta`/`alecrae-web` systemd units on 161); (2) set PTR record on 161 to `mail.alecrae.com`; (3) confirm email arrives in Gmail inbox; (4) run MTA idempotency check (same message_id → no duplicate).
