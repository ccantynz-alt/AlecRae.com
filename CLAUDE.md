...
| 23 | **Migrations cover only 16 of 136 schema tables** — a `db:migrate`-only fresh DB lacks 120 tables incl. login-critical `passkeys`/`refresh_tokens`/`connected_accounts`; `db:push` is also broken (`drizzle-kit` can't resolve `./*.js` schema imports). migrate+seed works, but the app can't fully run (login/inbox/Tier 5–8) until this is closed | HIGH | 2026-06-11 | OPEN — needs full migration backfill or a drizzle-kit resolver fix; awaiting Craig's go-ahead on approach |
