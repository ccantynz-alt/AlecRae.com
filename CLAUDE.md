...
**Current focus:** Continuing straight through the audit's roadmap per Craig's standing instruction not to pause for check-ins on directed work. Shipped since the Big Five (issues #88-92 above): real read/starred/folder state for archive/delete/star/snooze (migration 0006), undo-send wired up (was dead code), the Domains page showing real DNS records instead of hard-coded wrong ones, a translate.ts cross-tenant cache leak, the send-time engagement loop closed, OAuth/IMAP token encryption at rest (issue #80), and sentiment-timeline.ts rebuilt on real Claude analysis — which surfaced a much bigger bonus find: 5 Tier-7 AI routers (sentiment-timeline, context-intelligence, knowledge-graph, productivity-analytics, scheduling-intelligence) were reading a context key nothing ever sets, so every query in all 5 was scoped by `accountId=undefined` and most likely erroring on every call. Remaining roadmap: Google Workspace import UI, outbound email design/HTML framework (Tiptap install + template system), wiring the already-built local WebGPU grammar engine, killing the remaining `Math.random()` surfaces in ai-intelligence.ts (writing-coach, predictive-actions), the AI-rules execution engine (rules save but never run against mail), and password reset (blocked on transactional email — see issue #83). Full findings + prioritized roadmap in the session's audit artifact — not yet transcribed into a docs/ file. Mail plan unchanged: Phase 1 (MTA bring-up on 158) is next on the mail track.
