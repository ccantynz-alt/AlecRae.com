...
**Current focus:** Continuing straight through the audit's roadmap per Craig's standing instruction not to pause for check-ins on directed work. 22 fixes shipped this session (issues #84-101 above, full list). Beyond the headline items already noted: voice-clone plan gating + honest AI-unavailable errors, ai-writing.ts's fallback paths stopped faking confidence scores, dictation wired into compose end-to-end (real Whisper + Claude, previously zero UI), and — the largest bonus find of the session — a full audit of every `requireScope()` prefix against what `scopesForRole()` actually grants turned up SIX MORE missing baseline scopes (`agent:*`, `collaborate:*`, `dictation:*`, `templates:*`, `translate:read`, `programs:*`, `calendar:*`, `analytics:read` for members) that were silently 403ing the AI Inbox Agent, team chat, dictation, Templates, Translate, Programs, compose's calendar-slot assistant, and the read side of every Tier-7 AI router, for every session (issue #100). Remaining roadmap: Google Workspace import UI, outbound email design/HTML framework (Tiptap install + template system), the `smart_label_rules` half of the rules-execution gap (separate system from `email_rules`, not covered by #93), and password reset (blocked on transactional email — see issue #83). Full findings + prioritized roadmap in the session's audit artifact — not yet transcribed into a docs/ file. Mail plan unchanged: Phase 1 (MTA bring-up on 158) is next on the mail track.
