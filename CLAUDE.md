...
| 79 | Gmail/Outlook initial sync after OAuth connect is fire-and-forget — errors only go to console, account stays `active`, user sees a silently empty inbox. Surface sync status/last error on the connected account (UI + `lastSyncAt`/`lastError`). | MEDIUM | 2026-07-14 | OPEN |
...
**Shipped 2026-07-14 (2nd wave):** Vapron DNS auto-config provider — domains page can now push SPF/DKIM/DMARC/MX/bounce records into a Vapron-hosted zone with ZERO user credentials (server-side VAPRON_API_KEY via the existing tRPC client; new `apps/api/src/lib/dns-providers/vapron.ts`, provider added to route schema + wizard). Deliverability audit completed → issue #82 (the 5-gap pre-send gate).
...
**Current focus:** 2026-07-14 morning audit (Craig-ordered "full order"): found the box serving a **July-4 build** (10-day deploy drift, issue #78) and **Redis never installed** on Jarvis — both fixed and deployed same morning (box reset to origin/main + redis-server installed + full ritual). Root-caused the "admin sees Upgrade prompts" class: founder plan reconcile only ran at login for the home account — `/me` and `switch-workspace` now reconcile the active workspace, `/me` returns `isFounder` + scopes, PlanGate honors `isFounder`, and a Session Diagnostics card on /settings shows exactly what the server thinks of the current login. Domains page added to the sidebar (was command-palette-only). Remaining suspected blocker for Gmail connect / Workspace import is Google Cloud Console config (redirect URIs / restricted scopes / test users) — verify live with Craig (Next Action 6b). Mail plan unchanged: Phase 1 (MTA bring-up on 158) is next on the mail track.
