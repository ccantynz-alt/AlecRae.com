...
| 171 | **AlecRae⇄Vapron mail integration (THE business-email path — see `docs/infra/alecrae-vapron-mail-integration.md`).** **AlecRae-side adapters BUILT 2026-08-08:** (a) outbound = MTA `RELAY_PROVIDER=vapron` provider → POSTs Vapron `email-send` `/v1/messages` (Bearer token; uses the message From not the VERP return-path so Vapron's FROM-verify passes; 403→bounce, 5xx→defer); (b) inbound = public `POST /v1/inbound/vapron` verifying Vapron's `X-Vapron-Signature` HMAC + replay guard, resolving account by recipient domain, storing via `storeReceivedEmail`. 31 tests. **Remaining to go live:** (c) deploy + config — set `INBOUND_WEBHOOK_SECRET` on apps/api (= Vapron's `INBOUND_SINK_SECRET`); run the MTA worker on Jarvis with `RELAY_PROVIDER=vapron` + `VAPRON_EMAIL_SEND_URL/TOKEN`; (d) **Craig/Vapron:** set `INBOUND_SINK_URL/SECRET/DOMAINS` + restart `vapron-email-receive` (dormant bridge, rejecting all inbound since ~2026-08-05), register AlecRae domains in `vapron-email-domain`. (e) Follow-up: inbound attachments + SPF/DKIM verdicts not yet passed through (`storeReceivedEmail` can't carry them — body stored, attachments logged). Prove davenroe.com loop first. Phase 1 Redis DONE. | HIGH |
